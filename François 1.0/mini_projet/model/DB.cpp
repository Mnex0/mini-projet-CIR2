...
void BDD::inserePlan(Plan &plan)
{
    ...
    stmt->execute("insert into plan values (null,'" // null => auto_increment
                  + plan.getNomProjet() + "'," + std::to_string(plan.getVersion()) + "," + std::to_string(plan.getOrientation()) + ",sysdate())");
    for (auto &mur : plan.getMurs())
    {
        ...
        stmt->execute("insert into mur values (" + std::to_string(id) + ",'" + mur.getNomMur() + "','" + mur.getCouleur() + "'," + std::to_string(mur.getEpaisseur()) + ")");
        // Ajout point
        for (auto &pt : mur.getPoints())
        {
            stmt->execute("insert into point values (" + std::to_string(id) + ",'" + mur.getNomMur() + "'," + std::to_string(pt.getNum()) + "," + std::to_string(pt.getX()) + "," + std::to_string(pt.getY()) + ")");
        } // fin insert point
...
void BDD::ajouterMurs(Plan &plan)
{
    ...
    sql::Statement *stmt = con->createStatement();
    sql::ResultSet *res = stmt->executeQuery(
        "select * from mur where id_plan=" + std::to_string(plan.getIdPlan()));
    while (res->next())
    {
...
void BDD::ajouterPoints(Mur &mur)
{
    ...
    sql::Statement *stmt = con->createStatement();
    sql::ResultSet *res = stmt->executeQuery(
        "select * from point where id_plan=" + std::to_string(mur.getIdPlan()) + " and nom_mur='" + mur.getNomMur() + "'" + " order by num");
    while (res->next())
    {
